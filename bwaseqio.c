...
static int bwa_read_bam_pe(bwa_seqio_t *bs, int n_needed, int *n, int is_comp, int trim_qual, bwa_seq_t *seqs[2])
{
	bwa_seq_t *p;
	int n_seqs, l, i;
	long n_trimmed = 0, n_tot = 0;
	bam1_t *b;
	int res, read1_flag = 0;
...
//add by zhangyong2
bwa_seq_t *bwa_read_seq_se(bwa_seqio_t *bs, int n_needed, int *n, int mode, int trim_qual)
{
	bwa_seq_t *seqs, *p;
	kseq_t *seq = bs->ks;
	int n_seqs, l, i, is_comp = mode&BWA_MODE_COMPREAD, is_64 = mode&BWA_MODE_IL13, l_bc = mode>>24;
	...
	while ((l = kseq_read_gaea(seq)) >= 0) {
		...
		//fprintf(stderr, "%s\t%s\t%s\t%s\n", seq->name.s, seq->seq.s, seq->comment.s, seq->qual.s);
		if (is_64 && seq->qual.l)
			for (i = 0; i < seq->qual.l; ++i) seq->qual.s[i] -= 31;
		if (seq->seq.l <= l_bc) continue; // sequence length equals or smaller than the barcode length
		p = &seqs[n_seqs++];
		...
		n_tot += p->full_len;
		p->seq = (ubyte_t*)calloc(p->full_len, 1);
		p->sample_id = atoi(seq->comment.s);
		//fprintf(stderr, "atoi sample ID : %d\n", p->sample_id);
...
//    soapnuke_filter(p, filter_opt, 0);
	...
	while ((l = kseq_read_gaea(seq)) >= 0) {
		...
		//fprintf(stderr, "%d\t%s\t%s\t%s\t%s\n", read1_flag, seq->name.s, seq->seq.s, seq->comment.s, seq->qual.s);
		if (is_64 && seq->qual.l)
			for (i = 0; i < seq->qual.l; ++i) seq->qual.s[i] -= 31;
		if (seq->seq.l <= l_bc) continue; // sequence length equals or smaller than the barcode length
		p = &seqs[read1_flag][n_seqs];
		...
		p->seq = (ubyte_t*)calloc(p->full_len, 1);
		p->sample_id = atoi(seq->comment.s);
		//fprintf(stderr, "sample id:%d\tcomment:%s\n", p->sample_id, seq->comment.s);
		for (i = 0; i != p->full_len; ++i)
			p->seq[i] = nst_nt4_table[(int)seq->seq.s[i]];
		if (seq->qual.l) { // copy quality
			p->qual = (ubyte_t*)strdup((char*)seq->qual.s);
...
//add by zhangyong2, need more development
/*
int bwa_read_seq_mix(bwa_seqio_t *bs, int n_needed, int *n, int mode, int trim_qual, bwa_seq_t **seqs[3], int *pe_count, int *se_count)
{
	bwa_seq_t *p;
	kseq_t *seq = bs->ks;
	int n_seqs, l, i, is_comp = mode&BWA_MODE_COMPREAD, is_64 = mode&BWA_MODE_IL13, l_bc = mode>>24;
	int buff_size = n_needed, buff_index = 0;
	long n_trimmed = 0, n_tot = 0;
	...
	n_seqs = 0;
	//seqs = (bwa_seq_t*)calloc(n_needed, sizeof(bwa_seq_t));
	seqs[0] = (bwa_seq_t**)calloc(n_needed, sizeof(bwa_seq_t*));//PE1
	seqs[1] = (bwa_seq_t**)calloc(n_needed, sizeof(bwa_seq_t*));//PE2
	seqs[2] = (bwa_seq_t**)calloc(n_needed, sizeof(bwa_seq_t*));//SE
	bwa_seq_t *seqs_buff = (bwa_seq_t*)calloc(buff_size, sizeof(bwa_seq_t));//buff
	while ((l = kseq_read_gaea(seq)) >= 0) {
		if ((mode & BWA_MODE_CFY) && (seq->comment.l != 0)) {
			// skip reads that are marked to be filtered by Casava
			char *s = index(seq->comment.s, ':');
		...
		//zhangyong2 add for mix PE and SE reads
		if(buff_index == buff_size)
		{
			...
			{
				//PE
				if(0 == strcmp(seqs_buff[index]->name, seqs_buff[index + 1]->name))
				{
					seqs[0][n_seqs] = &seqs_buff[index];
					seqs[1][n_seqs++] = &seqs_buff[index + 1];
					index += 2;
					*pe_count += 2;
				}
				//SE
