...
static int usage()
{
	...
	fprintf(stderr, "         filter        filter reads in fastq\n");
	fprintf(stderr, "         mem           BWA-MEM algorithm\n");
	fprintf(stderr, "         fastmap       identify super-maximal exact matches\n");
	fprintf(stderr, "         pemerge       merge overlapping paired ends (EXPERIMENTAL)\n");
	fprintf(stderr, "         alnpe         gapped/ungapped alignment && generate alignment (paired ended)\n");
	fprintf(stderr, "         alnse         gapped/ungapped alignment && generate alignment (single ended)\n");
...
int main(int argc, char *argv[])
{
	...
	else if (strcmp(argv[1], "alnpe") == 0) ret = bwa_alnpe(argc-1, argv+1);
	else if (strcmp(argv[1], "alnse") == 0) ret = bwa_alnse(argc-1, argv+1);
